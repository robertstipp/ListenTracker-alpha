...
const AudioPlayHead = ({position, size}) => {
   const outerCircleRadius = size * .45;
...
const Player = () => {
  ...
  return (
    ...
    <div id="artContainer">
      {albumImageRef &&
      <AudioPlayHead position={trackPosition} size={albumImageRef.current?.width * 1.2} />
      }
      <img onLoad={handleLoadedImageData} ref={albumImageRef} id="albumArt" src="/albumArt" alt="" />  
    </div>
    <audio controls id="myAudio" onLoadedData={handleLoadedAudioData} onTimeUpdate={handleTimeUpdate} src="/music" ></audio>
    <div>
